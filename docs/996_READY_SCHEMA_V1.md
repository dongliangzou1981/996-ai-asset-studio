...
Sprint 15A adds optional quality report files:

- `component_quality_report.json`
- `component_quality_report.html`

They are generated by Component Processing for new packages. Older packages without these files remain valid.
...
| --- | --- | --- |
| `schema_version` | string | Must be `"1.0"` |
| `report_type` | string | Must be `"component_quality"` |
| `coordinate_space` | string | Must be `"ui_preview_pixels"` |
| `resolution` | object | Width and height of `ui_preview` |
| `total_components` | integer | Non-negative component and candidate count |
| `category_counts` | object | Counts for button, icon, frame, tab, slot, input, panel, background |
| `category_area` | object | Raw area by category |
| `coverage` | object | Total, covered, and uncovered area metrics |
| `suspected_missing_regions` | array | Coarse low-coverage regions for manual review |
| `components` | array | Component and candidate records used by the report |
...
The report is analytical only. It does not mean current slicing is production-grade semantic recognition.
...
- optional `component_quality_report.json` structure when present
